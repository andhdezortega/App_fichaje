...
public class Autenticacion {
    ...
    public static boolean verificarCredenciales(String correo, String password) {
        ...
        HttpSession session = request.getSession(true); // << primero obtén la sesión

        Map<String, String> userInfo = usuarioDAO.obtenerInfoUsuarioPorCorreo(correo);
        String estadoGuardado = userInfo.get("descripcion"); // o "estado_actual" según tu campo
        if (estadoGuardado != null && !estadoGuardado.isEmpty()) {
            session.setAttribute("estadoUsuario", estadoGuardado);
        } else {
