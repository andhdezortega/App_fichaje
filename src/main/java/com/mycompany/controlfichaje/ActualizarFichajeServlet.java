...
public class ActualizarFichajeServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        try {
            ...
            boolean estado = request.getParameter("estado") != null; // si el fichaje está cerrado
            int aprobadas = 0;
            String aprobadasParam = request.getParameter("aprobadas");
            if (aprobadasParam != null && !aprobadasParam.isEmpty()) {
                aprobadas = Integer.parseInt(aprobadasParam);
            }

            FichajeModel f = new FichajeModel(id, nombre, apellido, rol, fecha, entrada, salida, descanso, comida, horasSemanales, estado);
            f.setAprobadas(aprobadas); // <-- añadimos la columna aprobadas

            // Opcional: actualizar estado de horas extra si viene del formulario
            String estadoHorasExtra = request.getParameter("estadoHorasExtra");
            if (estadoHorasExtra != null) {
                f.setEstadoHorasExtra(estadoHorasExtra);
            }
