...
public class UsuarioDAO {
    ...
    // Obtener rol y descripcion por correo
    ...
    //registrar tiempos de produccion
    public static void registrarTiempo(int usuarioId, String estado, long minutos) {
    String sql = "INSERT INTO registro_tiempo(usuario_id, estado, minutos, fecha) VALUES(?, ?, ?, CURDATE())";
    try (Connection con = DatabaseConnection.getConnection();
        PreparedStatement ps = con.prepareStatement(sql)) {
        ps.setInt(1, usuarioId);
        ps.setString(2, estado);
        ps.setLong(3, minutos);
        ps.executeUpdate();
    } catch (SQLException e) {
        e.printStackTrace();
    }
